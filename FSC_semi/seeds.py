...
balance = pd.read_table("seeds.data",sep="\s+", header=None)
data= balance.iloc[:, :7].values
label=balance.iloc[:,7].values#2
dim=data.shape[1]
...
with tf.Session() as sess:
    ...
    num_cla=[np.sum(label==j) for j in range(k)]
    num_cla=np.sort(num_cla)
    g=np.array([i for i in range(k)])
    g[list(np.argsort(num_clu*pro_clu))]=[i for i in range(k)]
    num_cla=num_cla[g]
    mum_cla=num_cla/np.sum(num_cla)
